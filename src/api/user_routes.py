...
async def create_user(user_schema: UserSchema, session = Depends(get_session)):
...
async def update_password(user_schema: UserUpdateSchema, session = Depends(get_session)):
...
async def delete_user(user_schema: UserSchema, session = Depends(get_session)):
    user = session.query(User).filter(User.email== user_schema.email).first()
    if not user:
        raise HTTPException(status_code=404, detail="Usuário não encontrado")
    else:
